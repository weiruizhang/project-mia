	 	...
	 	"checkForm":function(options){
			...
			var _val=true;
	 		var defaults={
	 			...
	 			// 提示信息
	 			tip_success:'格式正确', //验证成功提示
	 			tip_required:'不能为空',
	 			tip_email:'邮箱格式有误',
	 			tip_user:"必须是中文",
	 			tip_phone:'请输入正确的手机号码，且为11位纯数字格式',
	 			...
	 			reg_password:/^(?=.*?[a-zA-Z])(?=.*?[0-6])[!"#$%&'()*+,\-./:;<=>?@\[\\\]^_`{|}~A-Za-z0-9]{6,14}$/
	 		...
	 		var showMsg=function(obj,msg,mark){
	 			...
	 			var _html="<span id='errormsg' style='font-size:12px;color:#f450a2;'>"+msg+"</span>"
	 			$(obj).after(_html);
	 			return mark;
	 		}
